...
import { 
  Search, Wand2, ArrowLeft, MoreVertical, Pencil, Trash2 
} from 'lucide-react'
import {
...
export function SearchBar() {
  ...
  const executeSearch = () => {
    ...
    if (pathname === '/chat') router.replace(`/chat?q=${encodeURIComponent(query)}`);
    else router.push(`/vault?q=${encodeURIComponent(query)}`);
  };
      ...
      <>
        ...
        <Dialog open={isRenameOpen} onOpenChange={setIsRenameOpen}>
            <DialogContent>
                ...
                <div className="py-2">
                    <Input value={newTitleInput} onChange={(e) => setNewTitleInput(e.target.value)} placeholder="Chat Title" />
                </div>
                <DialogFooter>
    ...
    <>
      <div className="w-full flex items-center gap-2">
        <div className="relative flex-1 min-w-0"> 
          ...
          <Input
            type="search"
            placeholder={pathname === '/chat' ? "Search conversations..." : "Search..."}
      ...
      <QandADialog 
        isOpen={isQnOpen} onOpenChange={setIsQnOpen} query={query} 
        answer={answer} isLoading={isLoadingAnswer} sources={sources} 
        onSourceClick={(id) => { setIsQnOpen(false); setSelectedItemId(id); setIsDetailsOpen(true); }}
      />
