...
import { SendHorizontal, Bot, LoaderCircle, AlertCircle } from 'lucide-react'
...
export function ChatInterface({ chatId, initialMessages }: ChatInterfaceProps) {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('/api/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chatId, message: userMessage }) });
        ...
        <div className="max-w-3xl mx-auto px-4 py-6 min-h-full flex flex-col justify-end">
                    ...
                    {messages.map((msg) => (
                        <div key={msg.id} className={cn("flex gap-4 w-full", msg.role === 'user' ? "justify-end" : "justify-start")}>
                            ...
                            <div className={cn(
                                ...
                                msg.role === 'user' ? "bg-stone-100 text-stone-900 rounded-br-sm" : "bg-transparent text-stone-800 pl-0 pt-0"
                            )}>
                                {msg.content}
                            </div>
                        </div>
            ...
            {isLoading && (
                <div className="flex gap-4 mt-8">
                    ...
                    <div className="py-2"><span className="flex gap-1"><span className="w-1.5 h-1.5 bg-stone-400 rounded-full animate-bounce [animation-delay:-0.3s]"></span><span className="w-1.5 h-1.5 bg-stone-400 rounded-full animate-bounce [animation-delay:-0.15s]"></span><span className="w-1.5 h-1.5 bg-stone-400 rounded-full animate-bounce"></span></span></div>
                </div>
            )}
            {error && (<div className="flex justify-center my-4"><div className="bg-red-50 text-red-600 px-4 py-2 rounded-lg text-sm flex items-center gap-2 border border-red-100"><AlertCircle className="h-4 w-4" />{error}</div></div>)}
