    ...
    import { MoreHorizontal, Trash2, Edit, FolderInput } from 'lucide-react'
    import { ItemDetailsDialog } from './item-details-dialog'
    import { MoveToCollectionDialog } from './move-to-collection-dialog'

    type VaultItem = { id: number; processed_title: string | null; processed_summary: string | null; processed_tags: string[] | null; };
    type Collection = { id: number; name: string };

    // FIX: We pass collections as a prop now
    export function VaultGrid({ initialItems, collections }: { initialItems: VaultItem[], collections: Collection[] }) {
      ...
      // State for the "Move to Collection" feature
      ...
      const handleItemUpdate = (updatedItem: any) => { setItems(items.map(item => item.id === updatedItem.id ? updatedItem : item)); handleCloseDetails(); }
      ...
      const handleItemMoved = (itemId: number, collectionId: number | null) => {
        // If we are on a collection page, removing an item should make it disappear from the view.
        ...
        <>
          {items && items.length > 0 ? (
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {items.map((item) => (
                <Card key={item.id} className="bg-white border-slate-200 shadow-sm flex flex-col transition-all hover:shadow-lg hover:-translate-y-1">
                  <div className="flex-grow cursor-pointer" onClick={() => handleOpenDetails(item.id)}>
                    <CardHeader><CardTitle className="font-serif text-lg font-semibold">{item.processed_title || "Untitled"}</CardTitle></CardHeader>
                    <CardContent><p className="text-sm text-muted-foreground line-clamp-4">{item.processed_summary || "No summary."}</p></CardContent>
                  </div>
